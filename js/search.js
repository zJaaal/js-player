...
const drawSearchResult = async (e) => {
  ...
  if (!tracks.data.data.length) {
    searchContent.innerHTML = `<div class="flex center">
  ...
  tracksResult.innerHTML = tracks.data.data
    .slice(0, 5)
    .reduce((acc, track) => acc + searchTracklistTemplate(track), "");
  ...
  albumsResult.innerHTML = Object.entries(albums)
    .slice(0, 8)
    .reduce((acc, album) => acc + albumlistTemplate(album[1]), "");
  ...
  artistsResult.innerHTML += Object.entries(artists)
    .slice(0, 12)
    .reduce((acc, artist) => acc + artistlistTemplate(artist[1]), "");
