...
export function drawRightAside(artists, list) {
  ...
  function listener(evt) {
    ...
    const delIcon = evt.composedPath().find((path) => path.tagName === "I");
    if (!artist) return;
    ...
    if (delIcon && !list.length) {
      ...
      drawRightAside(
        artists,
        Object.keys(artists).sort(() => Math.random() - 0.6)
      );
    } else if (delIcon) {
      ...
      drawRightAside(
        artists,
        list.filter((a) => a != artist.dataset.artistid)
      );
  ...
  if (!list.length) {
    ...
    ul.innerHTML = list
      .slice(0, 7)
      .reduce((acc, key) => acc + rightAsideTemplate(key, artists), "");
  }
